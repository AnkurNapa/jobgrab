...
const KEY = "jobs";
const SCHEMA_VERSION = 2; // bump when the on-read migration shape changes

// Teal-style pipeline.
export const STATUSES = [
  "bookmarked", "applying", "applied", "interviewing", "negotiating", "accepted", "closed",
];
...
export async function saveJob(partial, opts = {}) {
  ...
  if (k) {
    const idx = jobs.findIndex((j) => keyOf(j) === k);
    if (idx !== -1) {
